(() => {
  ...
  async function runGroq(action) { const input = document.querySelector("#wac-ai-input"); const status = document.querySelector("#wac-ai-status"); const result = document.querySelector("#wac-ai-result"); if (!input.value.trim()) { status.textContent = "Paste or type some text first."; return; } status.textContent = "Groq is thinking..."; result.hidden = true; const response = await chrome.runtime.sendMessage({ type: "WAC_GROQ", action, text: input.value }); if (!response?.ok) { status.textContent = response?.error || "The AI request failed."; return; } result.querySelector("pre").textContent = response.result; result.hidden = false; status.textContent = "Done."; }
